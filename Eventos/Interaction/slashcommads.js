...
module.exports = {
	...
	async execute(interaction, client) {
		if (interaction.isCommand()) {
			...
			// Verificar si el comando pertenece a un plugin
			if (cmd.plugin && client.pluginManager) {
				const isPluginEnabled = await client.pluginManager.isPluginEnabled(
					interaction.guildId,
					cmd.plugin,
				);
				...
				if (!isPluginEnabled) {
					return interaction.reply({
						content: `❌ El plugin **${cmd.plugin}** está deshabilitado en este servidor.`,
						ephemeral: true,
					});
				}
			}
